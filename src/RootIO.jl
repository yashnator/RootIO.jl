...
function _makeTTree(file::CxxWrap.CxxWrapCore.CxxPtr{ROOT.TFile}, name::String, title::String, branch_types, branch_names)
    ...
    for i in eachindex(branch_types)
        ...
        elseif branch_types[i] == Bool
            ...
            curr_branch = ROOT.Branch(tree, string(branch_names[i]), Ref(one(branch_types[i])), 100, 99)
            push!(current_branches, curr_branch)
        end
    end
...
Creates a new ROOT TTree with branches of given type or types or types from given data (data is not written to the tree).
...
function TTree(file::CxxWrap.CxxWrapCore.CxxPtr{ROOT.TFile}, name::String, title::String; kwargs...)
    branch_types = []
    branch_names = []
    # Filter all branch size variables and add them to corresponding array specification
    keys_to_filter = []
    for value in values(kwargs)
        if isa(value, Tuple)
            push!(keys_to_filter, value[2])
        end
    end
    for (key, value) in kwargs
        if key ∉ keys_to_filter
